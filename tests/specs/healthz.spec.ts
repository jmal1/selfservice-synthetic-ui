...
test('healthz_returns_ok', async ({ request }, testInfo) => {
	meta(testInfo, {
		...
		description:
			'Anonymous GET /healthz through Caddy returns 200 with status ok/healthy/up. Cheapest possible end-to-end probe. If this fails alongside the API healthz, the whole stack is down. If only this fails, Caddy or DNS for the configured SYNTHETIC_BASE_URL is the issue.',
		severity: 'critical',
		runbook:
