...
// https://s3.example.test using a presigned URL. That path has failure
...
const S3_ORIGIN = process.env.SYNTHETIC_S3_ORIGIN ?? 'https://s3.example.test';
...
test('minio_upload_endpoint_reachable', async ({ authedPage: page }, testInfo) => {
	meta(testInfo, {
		...
		description:
			...
			'unsigned cross-origin PUT to the configured S3 origin. Proves the browser can reach MinIO over a ' +
			...
			'cert-manager and the DNS credential), MinIO is down, or the ' +
